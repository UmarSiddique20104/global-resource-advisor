...
export const McaTable = () => {
    ...
    const handleView = (slug: string) => {
        push(`/mca-company/${slug}?mode=view`);
    };

    const handleEdit = (slug: string) => {
        push(`/mca-company/${slug}?mode=edit`);
    };
    const handleSave = () => {
        // Save logic here
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-8">
            <div className="flex flex-col gap-y-6">
                <div className="flex w-full flex-wrap items-center px-3 gap-y-2  sm:flex-row  justify-between sm:px-6">
                    <h2 className="mb-0 font-inter text-lg font-bold text-charcoal">MCA Companies</h2>
                    <Button value="Add MCA Com" icon={<Add />} onClick={() => push('/mca-company/0?mode=create')} className=" !h-[45px] !w-fit !px-6 !py-2" />
                </div>
                    ...
                    <div className="datatables">
                        <DataTable
                            ...
                            columns={[
                                ...
                                { accessor: 'businessWebsite', title: 'Contact' },
                                {
                                    accessor: 'actions',
                                    title: 'Actions',
                                    render: (record) => (
