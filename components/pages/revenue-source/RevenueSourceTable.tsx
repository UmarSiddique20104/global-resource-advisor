...
const Revenue_Source_Table: React.FC = () => {
    ...
    const handleView = (id: string) => {
        push(`/revenue-source/${id}?mode=view`);
    };

    const handleEdit = (id: string) => {
        push(`/revenue-source/${id}?mode=edit`);
    };
    const handleSave = () => {
        // Save logic here
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-8">
            <div className="flex flex-col gap-y-6">
                <div className="flex w-full flex-wrap items-center justify-between gap-y-2  px-3  sm:flex-row sm:px-6">
                    <h2 className="mb-0 font-inter text-lg font-bold text-charcoal">Revenue Source Company</h2>
                    <Button value="Add Rev Source" icon={<Add />} onClick={() => push('/revenue-source/0?mode=create')} className=" !h-[45px] !w-fit !px-6 !py-2" />
                </div>
                ...
                <div className="panel">
                    <div className="datatables">
                        <DataTable
                            ...
                            records={recordsData}
                            columns={[
                                { accessor: 'revenueSourceCompanyName', title: 'Company Name' },
                                { accessor: 'revenueSourceCompanyType', title: 'Company Type' },
                                {
                                    accessor: 'actions',
                                    title: 'Actions',
                                    render: (record) => (
...
export default Revenue_Source_Table;
