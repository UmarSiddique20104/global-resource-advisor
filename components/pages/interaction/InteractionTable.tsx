...
import { toggleDeleteModel } from '@/store/themeConfigSlice';
...
export const InteractionTable = () => {
    ...
    const handleView = (slug: string) => {
        push(`/interaction/${slug}?mode=view`);
    };

    const handleEdit = (slug: string) => {
        push(`/interaction/${slug}?mode=edit`);
    };
    const handleSave = () => {
        // Save logic here
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-8">
            <div className="flex flex-col gap-y-6">
                <div className="flex w-full flex-wrap items-center justify-between gap-y-2  px-3  sm:flex-row sm:px-6 ">
                    <h2 className="mb-0 font-inter text-lg font-bold text-charcoal">Interactions</h2>
                    <Button value="Add Interaction" icon={<Add />} onClick={() => push('/interaction/0?mode=create')} className=" !h-[45px] !w-fit !px-6 !py-2" />
                </div>
                    ...
                    <div className="datatables">
                        <DataTable
                            ...
                            columns={[
                                ...
                                { accessor: 'contactInteractedWith', title: 'Contact' },
                                { accessor: 'date', title: 'Date' },
                                { accessor: 'graRepresentative', title: 'GRA Rep' },
                                { accessor: 'contactInteractedWith', title: 'Company Interacted ' },
