'use client';
import { AD_STATUS, MCA_COM_NAME } from '@/app/constants/dashboard';
import { clientAdData } from '@/app/dummy/clientAcData';
...
const AccountDebtor: React.FC<InteractionProps> = ({ id, mode }) => {
    const adStatus = Object.values(AD_STATUS);
    ...
    useEffect(() => {
        if (id) {
            const data = clientAdData.find((item) => item.slug === id);
            if (data) {
                //@ts-ignore
                setFormData(data);
            }
    ...
    const handleSave = () => {
        ...
        push('/account-debtor/0?mode=edit');
        back();
    };
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-8">
            ...
            <div className="flex flex-col gap-6">
                <div className="flex w-full items-center  justify-between sm:px-6 ">
                    {mode === 'create' ? (
                        <h2 className="mb-0  font-inter text-lg font-bold text-charcoal">Add Client Account Debtor</h2>
                    ) : (
                        <div className="font-inter text-lg font-bold leading-[21.6px] text-charcoal">{isEditMode && mode === 'edit' ? 'Edit Client Account Debtor' : 'View Client Account Debtor'}</div>
                    )}
                </div>
                {/* hr line */}
            ...
            <div className="flex flex-col gap-6 pt-8">
                <div className="grid grid-cols-12 gap-6">
                    ...
                    <div className="max-sm:col-span-12 col-span-12 sm:col-span-4">
                        <FormInput type="text" value={formData.adCompanyName} name="adCompanyName" placeholder="AD company name" label="AD Company Name" onChange={handleInputChange} cap={true} />
                    </div>
                    <div className="max-sm:col-span-12 sm:col-span-4">
                        <SimpleDropdown data={adStatus} label="Status " onHandleClick={dropdownClick} />
                    </div>
