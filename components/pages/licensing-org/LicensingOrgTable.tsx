...
const Licensing_Org_Table: React.FC = () => {
    ...
    const handleView = (slug: string) => {
        push(`/licensing-org/${slug}?mode=view`);
    };

    const handleEdit = (slug: string) => {
        push(`/licensing-org/${slug}?mode=edit`);
    };
    const handleSave = () => {
        // Save logic here
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-8">
            <div className="flex flex-col gap-y-6">
                <div className="flex w-full flex-wrap items-center justify-between gap-y-2  px-3  sm:flex-row sm:px-6">
                    <h2 className="mb-0 font-inter text-lg font-bold text-charcoal">Licensing Organization</h2>
                    <Button value="Add Lic Org" icon={<Add />} onClick={() => push('/licensing-org/0?mode=create')} className=" !h-[45px] !w-fit !px-6 !py-2" />
                </div>
...
export default Licensing_Org_Table;
