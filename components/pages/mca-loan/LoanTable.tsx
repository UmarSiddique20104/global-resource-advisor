...
export const ClientLoanTable = () => {
    ...
    const handleView = (slug: string) => {
        push(`/mca-loan/${slug}?mode=view`);
    };

    const handleEdit = (slug: string) => {
        push(`/mca-loan/${slug}?mode=edit`);
    };
        ...
        <div className="px-4 py-4 sm:px-8 sm:py-8">
            <div className="flex flex-col gap-y-6">
                <div className="flex w-full flex-wrap items-center px-3 gap-y-2  sm:flex-row  justify-between sm:px-6">
                    <h2 className="mb-0 font-inter text-lg font-bold text-charcoal">Client MCA Loans</h2>
                    <Button value="Add MCA Loan" icon={<Add />} onClick={() => push('/mca-loan/0?mode=create')} className=" !h-[45px] !w-fit !px-6 !py-2" />
                </div>
                ...
                <div className="panel">
                    <div className="datatables">
                        <DataTable
                            ...
                            records={recordsData}
                            columns={[
                                { accessor: 'clientMcaLoanName', title: 'Client' },
                                { accessor: 'mcaCompanyName', title: 'MCA' },
                                { accessor: 'currentRtrBalance', title: 'Balance' },
                                // { accessor: 'typeOfContract', title: 'Type of Contract' },
                                {
                                    accessor: 'actions',
                                    title: 'Actions',
                                    render: (record) => (
                                        <div className="flex gap-x-3">
                                            ...
                                            <span
                                                ...
                                                onClick={() => handleDeleteClick(record.clientMcaLoanName)}
                                            >
                                                <Delete />
                                            </span>
                                        </div>
